...
pub fn emit_module(nix_expr: &str) -> Result<String, String> {
    ...
    let nixjs_rt_module = env!("NIXJS_RT_MODULE");
    let mut out_src = format!("import n from '{nixjs_rt_module}';\n");
    out_src += "export const __nixjs = n;\n";
...
fn emit_bin_op(bin_op: &ast::BinOp, out_src: &mut String) -> Result<(), String> {
    ...
    match operator {
        ...
        ast::BinOpKind::Add => emit_nixjs_bin_op(lhs, rhs, "add", out_src)?,
        ast::BinOpKind::Div => emit_nixjs_bin_op(lhs, rhs, "div", out_src)?,
        ast::BinOpKind::Mul => emit_nixjs_bin_op(lhs, rhs, "mul", out_src)?,
        ast::BinOpKind::Sub => emit_nixjs_bin_op(lhs, rhs, "sub", out_src)?,

        // Attrset
        ast::BinOpKind::Update => emit_nixjs_bin_op(lhs, rhs, "update", out_src)?,

        // Boolean
        ast::BinOpKind::And => emit_nixjs_bin_op(lhs, rhs, "and", out_src)?,
        ast::BinOpKind::Implication => emit_nixjs_bin_op(lhs, rhs, "implication", out_src)?,
        ast::BinOpKind::Or => emit_nixjs_bin_op(lhs, rhs, "or", out_src)?,

        // Comparison
        ast::BinOpKind::Equal => emit_nixjs_bin_op(lhs, rhs, "eq", out_src)?,
        ast::BinOpKind::Less => emit_nixjs_bin_op(lhs, rhs, "less", out_src)?,
        ast::BinOpKind::LessOrEq => emit_nixjs_bin_op(lhs, rhs, "lessEq", out_src)?,
        ast::BinOpKind::More => emit_nixjs_bin_op(lhs, rhs, "more", out_src)?,
        ast::BinOpKind::MoreOrEq => emit_nixjs_bin_op(lhs, rhs, "moreEq", out_src)?,
        ast::BinOpKind::NotEqual => emit_nixjs_bin_op(lhs, rhs, "neq", out_src)?,

        // List
        ast::BinOpKind::Concat => emit_nixjs_bin_op(lhs, rhs, "concat", out_src)?,
    }
    Ok(())
}

fn emit_nixjs_bin_op(
    lhs: &ast::Expr,
    rhs: &ast::Expr,
    nixjs_function: &str,
    out_src: &mut String,
...
fn emit_unary_op_kind(
    ...
    match operator {
        ast::UnaryOpKind::Invert => emit_nixjs_unary_op(operand, "invert", out_src),
        ast::UnaryOpKind::Negate => emit_nixjs_unary_op(operand, "neg", out_src),
    }
}

fn emit_nixjs_unary_op(
    operand: &ast::Expr,
    nixjs_function: &str,
    out_src: &mut String,
) -> Result<(), String> {
...
fn nix_value_from_module(
    ...
    let nixjs_attr = v8::String::new(scope, "__nixjs").unwrap();
    let nixjs: v8::Local<v8::Value> = namespace_obj.get(scope, nixjs_attr.into()).unwrap();
    ...
    let to_strict_fn: v8::Local<v8::Function> = try_get_js_object_key(scope, &nixjs, "toStrict")?
        .expect("Could not find the function `toStrict` in `nixjs`.")
        .try_into()
        .expect("`n.toStrict` is not a function.");
    let strict_nix_value = call_js_function(scope, &to_strict_fn, &[nix_value])?;

    js_value_to_nix(scope, &nixjs, &strict_nix_value)
}
...
fn create_eval_ctx<'s>(
    ...
    nixjs: &v8::Local<v8::Value>,
    script_path: &Path,
) -> Result<v8::Local<'s, v8::Object>, String> {
    let eval_ctx_type = get_nixjs_type(scope, nixjs, "EvalCtx")?;
...
fn js_value_to_nix(
    ...
    if let Some(value) = from_js_attrset(scope, nixjs, js_value)? {
        return Ok(value);
    }
    if let Some(value) = from_js_string(scope, nixjs, js_value)? {
        return Ok(value);
    }
    if let Some(value) = from_js_lazy(scope, nixjs, js_value)? {
        return Ok(value);
    }
    if let Some(value) = from_js_int(scope, nixjs, js_value)? {
        return Ok(value);
    }
    if let Some(value) = from_js_bool(scope, nixjs, js_value)? {
        return Ok(value);
    }
    if let Some(value) = from_js_float(scope, nixjs, js_value)? {
        return Ok(value);
    }
    if let Some(value) = from_js_list(scope, nixjs, js_value)? {
        return Ok(value);
    }
    if let Some(value) = from_js_path(scope, nixjs, js_value)? {
        return Ok(value);
    }
    if let Some(value) = from_js_lambda(scope, nixjs, js_value)? {
        return Ok(value);
    }
    todo!(
        "js_value_to_nix: {:?}",
...
fn from_js_int(
    ...
    if is_nixjs_type(scope, nixjs, js_value, "NixInt")? {
...
fn from_js_string(
    ...
    if is_nixjs_type(scope, nixjs, js_value, "NixString")? {
...
fn from_js_lazy(
    ...
    if is_nixjs_type(scope, nixjs, js_value, "Lazy")? {
        ...
        return Ok(Some(js_value_to_nix(scope, nixjs, &strict_value)?));
    }
    Ok(None)
}
...
fn from_js_bool(
    ...
    if is_nixjs_type(scope, nixjs, js_value, "NixBool")? {
...
fn from_js_float(
    ...
    if is_nixjs_type(scope, nixjs, js_value, "NixFloat")? {
        let value = try_get_js_object_key(scope, js_value, "value")?.ok_or_else(|| {
...
fn from_js_attrset(
    ...
    if is_nixjs_type(scope, nixjs, js_value, "Attrset")? {
        ...
        return Ok(Some(js_map_as_attrset(scope, nixjs, &underlying_map)?));
    }
    Ok(None)
}
...
fn from_js_list(
    ...
    if is_nixjs_type(scope, nixjs, js_value, "NixList")? {
        ...
        return Ok(Some(js_value_as_nix_array(scope, nixjs, &value_as_array)?));
    }
    Ok(None)
}
...
fn is_nixjs_type(
    ...
    let nixjs_type = get_nixjs_type(scope, nixjs, type_name)?;
    js_value.instance_of(scope, nixjs_type).ok_or_else(|| {
        format!(
...
fn js_value_as_nix_array(
    ...
    for idx in 0..length {
        ...
        match js_value_to_nix(scope, nixjs, &js_element) {
...
fn from_js_path(
    ...
    if !is_nixjs_type(scope, nixjs, js_value, "Path")? {
        return Ok(None);
    }
...
fn from_js_lambda(
    ...
    if !is_nixjs_type(scope, nixjs, js_value, "Lambda")? {
        return Ok(None);
    }
...
fn js_map_as_attrset(
    ...
    for idx in 0..js_map_array.length() / 2 {
        ...
        map.insert(
            ...
            js_value_to_nix(scope, nixjs, &value)?,
        );
    }
